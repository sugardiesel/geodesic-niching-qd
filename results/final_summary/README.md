...
This directory is the compact examiner-facing index to the authoritative corrected evidence. It
is derived from the completed searches in `results/phase5/` and
`results/phase6_open_robustness/`; generating it does not launch MAP-Elites.
...
- Corrected rolling-only geodesic support with endpoint-only centroids and `k=20`.
...
- `data/corrected_k20_baseline_b_vs_contribution_wilcoxon.csv`: headline comparison only.
- `data/rolling_only_k_sensitivity_report.csv`: authoritative permanent-reachability and final
  distance-ratio values for `k in {3,5,10,20,30}`.
- `data/phase1_horseshoe_u_bottom_detour_provenance.csv`: corrected shortest-path provenance and
  superseded Phase 1.5 landmark value.
...
The corrected mechanism is active: post-retraining assignment disagreement was 14.096% on the
horseshoe map and 13.780% on the open map. The structural audit nevertheless found mean permanent
starvation of 222.8 horseshoe and 204.3 open-map centroids out of 625 at the locked `k=20`.
...
the corrected horseshoe detour figure. The second command validates the `k=20` reachability rows
